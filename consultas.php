...
{  
    ...
    $sql = "SELECT SUM(`venta`) as v FROM `operacion` INNER JOIN tipoventa ON operacion.id_tipoVenta=tipoventa.id_tipoventa "
        ."WHERE operacion.id_tipoVenta=1 AND operacion.fecha='".$hoy;
...
{  
    $hoy=date('Y-m-d');
    $sql = "SELECT SUM(`venta`) as v FROM `operacion` INNER JOIN tipoventa ON operacion.id_tipoVenta=tipoventa.id_tipoventa "
        ."WHERE operacion.id_tipoVenta=2 AND operacion.fecha='".$hoy;
        $bd = obtenerConexion();
    $sentencia = $bd->query($sql);
    return $sentencia->fetchAll();
}
...
{  
    $hoy=date('Y-m-d');
    $sql = "SELECT SUM(`venta`) as v FROM `operacion` INNER JOIN tipoventa ON operacion.id_tipoVenta=tipoventa.id_tipoventa "
        ."WHERE operacion.id_tipoVenta=3 AND operacion.fecha='".$hoy;
...
{  
    $hoy=date('Y-m-d');
    $sql = "SELECT SUM(`venta`) as v FROM `operacion` INNER JOIN tipoventa ON operacion.id_tipoVenta=tipoventa.id_tipoventa "
        ."WHERE operacion.id_tipoVenta=4 AND operacion.fecha='".$hoy;
...
{  
    ...
    $sql = "SELECT SUM(monto) as p FROM cuentacorriente\n"  
          ." WHERE fecha='".$hoy." AND ". "operacion=0" ;
    //." GROUP BY articulo.id_articulo;";  
        $bd = obtenerConexion();
    $sentencia = $bd->query($sql);
    return $sentencia->fetchAll();
