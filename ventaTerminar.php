...
if( $_POST["msj"]==1)
{
        ...
        }else
        {
               ...
               // }    
                ...
                $hoy=date('Y-m-d');
                $sentencia = $bd->prepare("INSERT INTO operacion (id_operacion, id_usuario, venta, fecha, descuento, detalle, id_tipoVenta, detalles, id_cliente) VALUES (Null,?,?,?,?,?,?,?,?)");
                $sentencia->execute([$idSesion, $t, $hoy,$desc,$detallesdes,$tipov,$detallesop,$cliente]);
                ...
                $sentencia->execute([$var]);
                 ...
                 cBorrarSession();
                        if ($_POST["tipov"]==3 ) 
                        {
                                ...
                                $m=$t-$monto;
                                cuentaCoriente($op, $m,$hoy,$detalles_c,$c,$idSesion);  
