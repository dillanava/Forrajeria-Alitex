...
function cliente() {
    $bd = obtenerConexion(); // Suponiendo que obtenerConexion() es una función que devuelve una conexión a la base de datos.

    $sql = "SELECT id_cliente, apellido, nombre, telefono, email, direccion, localidad, estado FROM cliente WHERE activo = 1"; // Se agrega el campo de estado al SQL
    $sentencia = $bd->query($sql);
    $clientes = $sentencia->fetchAll(PDO::FETCH_OBJ); // Aquí se indica que se quieren los resultados como objetos

    foreach ($clientes as $cliente) {
        // Verificamos si las propiedades existen, si no, las establecemos como N/A
        if (!property_exists($cliente, 'email')) {
            $cliente->email = 'N/A';
        }
        if (!property_exists($cliente, 'direccion')) {
            $cliente->direccion = 'N/A';
        }
        if (!property_exists($cliente, 'localidad')) {
            $cliente->localidad = 'N/A';
        }
        if (!property_exists($cliente, 'estado')) { // Se agrega el campo de estado
            $cliente->estado = 'N/A';
        }
    }
...
function actualizarStockPedido($idArticulo, $cantidadPedido) {
    $bd = obtenerConexion();
    // Consultar el stock actual
    $sentencia = $bd->prepare("SELECT cantidad FROM stock WHERE id_articulo = ?");
    $sentencia->execute([$idArticulo]);
    $resultado = $sentencia->fetch(PDO::FETCH_OBJ);
    
    if($resultado) {
        $nuevoStock = $resultado->cantidad + $cantidadPedido;
        // Actualizar el stock con la nueva cantidad
        cambiarStock($idArticulo, $nuevoStock);
    }
}

function cuentaCoriente($op, $monto,$fecha,$detalles,$id_cliente,$usuario)
{
    $bd = obtenerConexion();
    $sql = "INSERT INTO cuentacorriente(id_cuentaCorriente, operacion, monto, fecha, detalles,id_cliente,id_usuario) VALUES (NULL,?,?,?,?,?,?)";
    $sentencia = $bd->prepare($sql);
    return $sentencia->execute([$op, $monto,$fecha,$detalles,$id_cliente,$usuario]);
}
...
function insertarcliente($apellido, $nombre, $telefono, $email, $direccion, $localidad, $estado) // Se agrega el parámetro $estado
{
    $bd = obtenerConexion();
    $sql = "INSERT INTO cliente (id_cliente, apellido, nombre, telefono, email, direccion, localidad, estado, activo)" // Se agrega el campo de estado al SQL
         . " VALUES (NULL,?,?,?,?,?,?,?,?)";

    $sentencia = $bd->prepare($sql);
    return $sentencia->execute([$apellido, $nombre, $telefono, $email, $direccion, $localidad, $estado, 1]); // Se agrega $estado al array de valores
}
...
function ingresarCierre($usuario,$fecha,$efectivo,$tarjeta, $corriente,$canje, $pago,$total,$ganancia)
{
    $bd = obtenerConexion();
    $sql="INSERT INTO cierrecaja (id_cierre, id_usuario, fecha, efectivo, tarjeta, cCorriente, canje, pagoEnCuentaC, Total, ganancia) VALUES"
    ." (NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $sentencia = $bd->prepare($sql);
    return $sentencia->execute([$usuario,$fecha,$efectivo,$tarjeta, $corriente,$canje, $pago,$total,$ganancia]);
}
...
function cierreCaja($where)
{
    ...
    . "usuario.apellido, usuario.nombre, fecha "
    . "FROM cierrecaja \n"
...
function modificarCliente($id, $a, $n, $t, $e, $d, $l, $estado) { // Se agrega el parámetro $estado
    $bd = obtenerConexion(); // Suponiendo que obtenerConexion() es una función que devuelve una conexión a la base de datos.

    $sql = "UPDATE cliente SET apellido = '".$a."', nombre = '".$n."', telefono = '".$t."', email = '".$e."', direccion = '".$d."', localidad = '".$l."', estado = '".$estado."' WHERE id_cliente = ".$id; // Se agrega el campo de estado al SQL
    $sentencia = $bd->query($sql);

    // Se devuelve true si la actualización fue exitosa, de lo contrario, devuelve false
    return $sentencia !== false;
}
...
function editarProducto($id,$nombre,$presentacion,$unidad,$categoria,$precioI,$precioF,$unidadv,$limite,$caducidad,$detalles)
